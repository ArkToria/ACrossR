...
pub fn empty_group_by_id(group_id: i64) -> anyhow::Result<()> {
...
pub fn update_group_by_id(group_id: i64, use_proxy: bool) -> anyhow::Result<()> {
    let lock = ACROSS_RPC.lock().unwrap();
    let channel = match &lock.endpoint {
        Some(c) => c.clone(),
        None => return Err(anyhow::anyhow!("No Endpoint")),
...
pub fn append_node_by_url(group_id: i64, url: String) -> anyhow::Result<i64> {
    let lock = ACROSS_RPC.lock().unwrap();
    let channel = match &lock.endpoint {
        Some(c) => c.clone(),
        None => return Err(anyhow::anyhow!("No Endpoint")),
...
pub fn append_node(group_id: i64, node_data: ffi::NodeData) -> anyhow::Result<i64> {
    let lock = ACROSS_RPC.lock().unwrap();
    let channel = match &lock.endpoint {
        Some(c) => c.clone(),
        None => return Err(anyhow::anyhow!("No Endpoint")),
...
pub fn append_group(group_data: ffi::GroupData) -> anyhow::Result<i64> {
    let lock = ACROSS_RPC.lock().unwrap();
    let channel = match &lock.endpoint {
        Some(c) => c.clone(),
        None => return Err(anyhow::anyhow!("No Endpoint")),
...
pub fn remove_group_by_id(group_id: i64) -> anyhow::Result<()> {
    let lock = ACROSS_RPC.lock().unwrap();
    let channel = match &lock.endpoint {
        Some(c) => c.clone(),
        None => return Err(anyhow::anyhow!("No Endpoint")),
...
pub fn remove_node_by_id(node_id: i64) -> anyhow::Result<()> {
    let lock = ACROSS_RPC.lock().unwrap();
    let channel = match &lock.endpoint {
        Some(c) => c.clone(),
        None => return Err(anyhow::anyhow!("No Endpoint")),
...
pub fn set_node_by_url(node_id: i64, url: String) -> anyhow::Result<()> {
    let lock = ACROSS_RPC.lock().unwrap();
    let channel = match &lock.endpoint {
        Some(c) => c.clone(),
        None => return Err(anyhow::anyhow!("No Endpoint")),
...
pub fn set_node_by_id(node_id: i64, node_data: ffi::NodeData) -> anyhow::Result<()> {
    let lock = ACROSS_RPC.lock().unwrap();
    let channel = match &lock.endpoint {
        Some(c) => c.clone(),
        None => return Err(anyhow::anyhow!("No Endpoint")),
...
pub fn set_group_by_id(group_id: i64, group_data: ffi::GroupData) -> anyhow::Result<()> {
    let lock = ACROSS_RPC.lock().unwrap();
    let channel = match &lock.endpoint {
        Some(c) => c.clone(),
        None => return Err(anyhow::anyhow!("No Endpoint")),
...
pub fn get_node_by_id(node_id: i64) -> anyhow::Result<ffi::NodeData> {
    let lock = ACROSS_RPC.lock().unwrap();
    let channel = match &lock.endpoint {
        Some(c) => c.clone(),
        None => return Err(anyhow::anyhow!("No Endpoint")),
...
pub fn get_group_by_id(group_id: i64) -> anyhow::Result<ffi::GroupData> {
    let lock = ACROSS_RPC.lock().unwrap();
    let channel = match &lock.endpoint {
        Some(c) => c.clone(),
        None => return Err(anyhow::anyhow!("No Endpoint")),
...
pub fn list_all_nodes(group_id: i64) -> anyhow::Result<ffi::NodeList> {
    let lock = ACROSS_RPC.lock().unwrap();
    let channel = match &lock.endpoint {
        Some(c) => c.clone(),
        None => return Err(anyhow::anyhow!("No Endpoint")),
...
pub fn count_nodes(group_id: i64) -> anyhow::Result<u64> {
    let lock = ACROSS_RPC.lock().unwrap();
    let channel = match &lock.endpoint {
        Some(c) => c.clone(),
        None => return Err(anyhow::anyhow!("No Endpoint")),
