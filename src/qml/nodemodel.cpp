...
void NodeModel::select(int64_t group_id) {
  auto node_list = std::make_unique<across::profile::NodeList>(
      across::profile::list_all_nodes(group_id));
...
bool NodeModel::setData(const QModelIndex &index, const QVariant &value,
                        int role) {
